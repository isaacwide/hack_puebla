...
document.addEventListener('DOMContentLoaded', async () => {
     ...
     * @param {string} status - El estado del sitio ('SEGURO', 'ADULTO', etc.)
     * @param {string} geminiResponse - La respuesta completa de Gemini
     */
    const displayResult = (status, geminiResponse) => {
        let titleColor, titleText, bgColor;
        
        // Ahora, el título se extrae de la primera línea de la respuesta de Gemini.
        // Si no hay una respuesta válida, se usa un título predeterminado.
        const firstLine = geminiResponse.split('\n')[0].replace('**', '').replace(/\*/g, '').trim() || 'Análisis Desconocido';
        titleText = firstLine;
        ...
        if (contentDisplay) {
            contentDisplay.innerHTML = `
                <div class="${bgColor} rounded-md p-4 mb-4">
                    <h2 class="text-lg font-semibold ${titleColor} mb-2">${titleText}</h2>
    ...
    const analyzeUrl = async (url) => {
        ...
        try {
            ...
            displayResult(data.tipo_de_incidente, data.gemini_response);
